...
├── workflow.py         # GitOps 层：工作流组合、策略选择
...
| 模块 | 职责 | 理论层次 |
|------|------|----------|
| `cli.py` | 解析命令，调用状态机，格式化输出 | - |
| `workflow.py` | 工作流组合、收敛策略、创造性设计 | **范畴论** |
| `fsm.py` | 定义状态、转移规则、验证合法性 | **类型论** |
| `git_ops.py` | 封装 git 操作，返回明确结果 | 实现层 |
...
| 层次 | 回答的问题 | 理论 |
|------|------------|------|
| 范畴论层 | "我们能创造出什么新模式？" | 组合、变换、创造 |
| 类型论层 | "这个操作允许发生吗？" | 约束、验证、安全 |
| 实现层 | "如何执行 git 命令？" | 副作用、IO |
...
详见 [GitOps 工作流设计](./gitops-workflow.md)
...
class WorkflowEngine:
    def __init__(self, repo_root: Path, strategy: Strategy):
        self.git_ops = GitOps(repo_root)
        self.fsm = StateMachine(RepoState.DIRTY)
        self.strategy = strategy
    
    def run_standard_workflow(self) -> WorkflowResult:
        """标准工作流：doc-check → sync → commit"""

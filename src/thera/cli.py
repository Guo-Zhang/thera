...
def main():
    ...
    refresh_parser = subparsers.add_parser("refresh", help="同步子模块并提交推送")
    refresh_parser.add_argument("--dry-run", action="store_true", help="预览模式")
    refresh_parser.add_argument("--repo", default=".", help="仓库根目录")

    auto_commit_parser = subparsers.add_parser("auto-commit", help="检测变更并提交推送")
    auto_commit_parser.add_argument(
        "--dry-run", action="store_true", help="仅显示变更，不提交"
    )
    auto_commit_parser.add_argument("--repo", default=".", help="仓库根目录")
    auto_commit_parser.add_argument(
        "--strategy",
    ...
    doc_check_parser = subparsers.add_parser("doc-check", help="检查文档一致性")
    doc_check_parser.add_argument(
        "--config", default="meta/profile/submodules.yaml", help="YAML 配置文件路径"
    )
    doc_check_parser.add_argument("--repo", default=".", help="仓库根目录")

    submodule_parser = subparsers.add_parser("submodule-sync", help="同步子模块")
    submodule_parser.add_argument("--check", action="store_true", help="检测远程更新")
    submodule_parser.add_argument(
        "--sync", metavar="PATHS", help="同步指定子模块（逗号分隔）"
    )
    submodule_parser.add_argument(
        "--sync-all", action="store_true", help="同步所有子模块"
    )
    submodule_parser.add_argument("--repo", default=".", help="仓库根目录")

    workflow_parser = subparsers.add_parser("workflow", help="工作流相关命令")
    workflow_subparsers = workflow_parser.add_subparsers(
        dest="workflow_command", help="子命令"
    )
...
def run_refresh(args):
    """运行 refresh - 同步子模块并提交推送"""
    repo_root = Path(args.repo)

    if args.dry_run:
        result = refresh(repo_root, dry_run=True)
        print(f"[预览模式] {result.message}")
    ...
    if result.success:
        if result.commit_sha:
            print(f"✓ 已提交并推送 ({result.commit_sha})")
        else:
...
def run_workflow_audit(args):
    ...
    if audit_report["state_distribution"]:
        ...
        for state_name, count in sorted(
            audit_report["state_distribution"].items(), key=lambda x: x[1], reverse=True
        ):
    ...
    if audit_report["errors"]:
        ...
        for err in audit_report["errors"]:
            error_name = (
                err["error"].name
                if hasattr(err["error"], "name")
                else str(err["error"])
            )
            event_name = (
                err["event"].name
                if hasattr(err["event"], "name")
                else str(err["event"])
            )
